...
class Spatial_Attention_layer(nn.Block):
    ...
    def __init__(self, **kwargs):
        ...
        with self.name_scope():    
            ...
            # below we defien the parameters to be trained
    ...
    def forward(self, x, spatial_attention):
        ...
        for time_step in range(num_of_timesteps):
            ...
            for k in range(self.K):
                ...
                # shape of T_k_with_at is (batch_size, #node, #node)
...
class Temporal_Attention_layer(nn.Block):
    ...
    def forward(self, x):
        ...
        # compute temporal attention scores
        # after x transpose, size = (batchsize, #time_step, #feature_per_node, #node )
        # shape is (batchsize, #time_step, #node)
        lhs = nd.dot(nd.dot(x.transpose((0, 3, 2, 1)), self.U_1.data()),
                     self.U_2.data())

        # after x transpose, size = (#feature_per_node, batchsize, #node, #time_step)
        # shape is (batchsize, #node, #time_step)
        rhs = nd.dot(self.U_3.data(), x.transpose((2, 0, 1, 3)))

        # shape is (batchsize, #time_step, #time_step)
        product = nd.batch_dot(lhs, rhs)
...
class ASTGCN_block(nn.Block):
    ...
    def forward(self, x):
        ...
        Parameters
        ----------
        x: mx.ndarray, shape is (batch_size, #node, #feature_per_node, #time_step)

        Returns
        ----------
        mx.ndarray, shape is (batch_size, #node, num_of_time_filters, #time_step)
        ...
        # x after reshape: (batchsize, #node * # feature_per_node, #time_step)
        # after multiply by time attention: (batchsize, #node * # feature_per_node, #time_step)
        #   it means that for each node each feature we have a time series
        #   here we use time attention to integrate the time series information
        # after second reshape: (batch_size, #node, #feature_per_node, #time_step)
        ...
        # spatial_At: (batchsize, #node, #node)
        spatial_At = self.SAt(x_TAt)
        # spatial_gcn: (batch_size, #node, self.num_of_filters, #time_step)
